...
def setup_nocturne_jobs(scheduler: Any) -> None:
    """Enregistre les 7 jobs nocturnes APScheduler v2.2 sur le scheduler fourni.

    Jobs planifiés sur la plage 00h00–06h00 (lun-dim) :
    ...
    nocturne_jobs = [
        ...
        (
            "relances_nocturne",
            lambda: send_relance(check_docs("jmpartners"), dry_run=False),
            2,
            30,
        ),
    ...
    logger.info(
        "setup_nocturne_jobs — %d jobs nocturnes enregistrés", len(nocturne_jobs)
    )
