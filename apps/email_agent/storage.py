...
_EMAIL_USER_ID = os.getenv("EMAIL_USER_ID", "")
if not _EMAIL_USER_ID:
    raise EnvironmentError(
        "EMAIL_USER_ID est requis mais absent. "
        "Définissez la variable d'environnement EMAIL_USER_ID."
    )


def get_supabase_client() -> Client:
    """Retourne un client Supabase authentifié via les variables d'environnement."""
...
def save_email(analyzed_email: dict) -> bool:
    """Sauvegarde un email analysé dans la table emails_analyzed.

    Args:
        analyzed_email: Dictionnaire contenant les champs subject, from, date,
            priority, category, summary, action, suggested_reply.

    Returns:
        True si l'insertion Supabase réussit, False en cas d'erreur.
    """
    try:
        ...
        logger.info("Email sauvegarde : %s", data["email_subject"][:50])
        return True
...
def calculate_and_save_kpis(emails_analyzed: list, temps_agent_sec: float) -> dict:
    """Calcule les KPIs du jour et les sauvegarde dans agent_weekly_stats.

    Args:
        emails_analyzed: Liste des emails analysés dans ce cycle.
        temps_agent_sec: Durée d'exécution de l'agent en secondes.

    Returns:
        Dictionnaire des KPIs calculés, ou dict vide en cas d'erreur.
    """
    try:
        client = get_supabase_client()
        ...
        logger.info("KPIs sauvegardes : %s", kpis)
        return kpis
...
def save_weekly_stats(stats: dict) -> bool:
    """Sauvegarde les stats hebdomadaires dans Supabase avec retry automatique.

    Args:
        stats: Dictionnaire des statistiques à persister dans agent_weekly_stats.

    Returns:
        True si l'insertion réussit, False en cas d'erreur.
    """
    try:
        client = get_supabase_client()
        client.table("agent_weekly_stats").insert(dict(stats)).execute()  # type: ignore[arg-type]
        ...
        logger.error("Erreur KPIs Supabase : %s", e)
        return False
